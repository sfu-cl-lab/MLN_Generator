...
DROP SCHEMA IF EXISTS imdb_MovieLens_setup; 
create schema imdb_MovieLens_setup;

create schema if not exists imdb_MovieLens_BN;
create schema if not exists imdb_MovieLens_CT;

USE imdb_MovieLens_setup;
...
WHERE
    (KEY_COLUMN_USAGE.TABLE_SCHEMA = 'imdb_MovieLens')
ORDER BY TABLE_NAME;
...
WHERE
    (COLUMNS.TABLE_SCHEMA = 'imdb_MovieLens'
        AND TABLES.TABLE_SCHEMA = 'imdb_MovieLens'
