...
CREATE TABLE RNodes_From_List AS SELECT DISTINCT rnid, CONCAT('imdb_MovieLens.',TABLE_NAME, ' AS ', pvid) AS Entries FROM
    RNodes_pvars 
UNION DISTINCT 
SELECT DISTINCT
    rnid, CONCAT('imdb_MovieLens.',TABLE_NAME, ' AS ', rnid) AS Entries
FROM
    RNodes 
union distinct 
select distinct
...
CREATE TABLE ADT_PVariables_From_List AS SELECT pvid, CONCAT('imdb_MovieLens.',TABLE_NAME, ' AS ', pvid) AS Entries FROM
    PVariables;
